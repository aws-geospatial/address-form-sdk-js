...
| `centerBounds`                  | `[[number, number], [number, number]]` | No       | -       | Supported-region box as `[[west, south], [east, north]]` in [longitude, latitude]. When set, search stays in-region (the typeahead bias is clamped into the box, so results stay in-region even if the user pans the map away); clicking the current-location button from a device outside the box shows a warning and leaves the field untouched, rather than returning the nearest in-region address; and the adjustable pin is kept in-region: if the user drags the pin outside the box, the map's adjustment bar shows an out-of-region warning, its "Undo adjustment" action returns the pin, and form submission is blocked until it is back in the region. Use for data providers that only cover part of the world (e.g. GrabMaps in Southeast Asia). |
